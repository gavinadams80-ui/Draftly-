...
export function generateCornerPostSVG(params?: DrawingParams): string {
  ...
  svg += callout(ledX + ledW / 2, ledY - 8, p.ledgerSize || 'C150x50x1.9', ledX + ledW / 2, ledY);
  ...
  svg += `<line x1="${postX + postD / 2}" y1="${postY + postH * 0.15}" x2="${postX + postD / 2}" y2="${postY + postH * 0.85}" stroke="${C_POST}" stroke-width="${M}" stroke-dasharray="3,2"/>`;
  svg += `<line x1="${postX + lip}" y1="${postY}" x2="${postX + lip}" y2="${postY + postH}" stroke="${C_POST}" stroke-width="${M}"/>`;
  svg += `<line x1="${postX + postD - lip}" y1="${postY}" x2="${postX + postD - lip}" y2="${postY + postH}" stroke="${C_POST}" stroke-width="${M}"/>`;
  svg += callout(postX + postD + 15, postY + postH / 2, p.postSize || 'C100x50x1.6', postX + postD, postY + postH / 2);
  ...
  for (let i = 0; i < 2; i++) {
    for (let j = 0; j < 2; j++) {
      const bx_pos = cx - 35 * sc + i * 70 * sc;
      const by_pos = bpY + 4 * sc;
  ...
  svg += dimH(cx - bpW / 2, gy + 14, cx + bpW / 2, gy + 14, `${bpDims.w}`, 6);
...
export function generateRafterLedgerSVG(params?: DrawingParams): string {
  ...
  const ledW = 200 * sc;
  const ledD = ledDims.d * sc;
  svg += `<rect x="${cx - ledW / 2}" y="${baseY}" width="${ledW}" height="${ledD}" fill="${C_LEDGER_FILL}" stroke="${C_LEDGER}" stroke-width="${T}"/>`;
  svg += `<line x1="${cx - ledW / 2 + ledW * 0.15}" y1="${baseY + ledD / 2}" x2="${cx + ledW / 2 - ledW * 0.15}" y2="${baseY + ledD / 2}" stroke="${C_LEDGER}" stroke-width="${M}" stroke-dasharray="3,2"/>`;
  svg += callout(cx, baseY + 12, p.ledgerSize || 'C150x50x1.9', cx, baseY);

  // Rafter (angled at pitch)
  ...
  svg += `<polygon points="${rSx},${rSy} ${rEx},${rEy} ${rEx + perpX},${rEy + perpY} ${rSx + perpX},${rSy + perpY}" fill="${C_LEDGER_FILL}" stroke="${C_LEDGER}" stroke-width="${T}"/>`;
  ...
  svg += `<line x1="${rSx}" y1="${rSy + perpY}" x2="${rSx + bmDepth * Math.cos(angle)}" y2="${rSy + perpY + bmDepth * Math.sin(angle)}" stroke="${C_LEDGER}" stroke-width="${T}"/>`;
  svg += `<line x1="${rSx + bmDepth * Math.cos(angle)}" y1="${rSy + perpY + bmDepth * Math.sin(angle)}" x2="${rSx + perpX * 0.3}" y2="${rSy + perpY}" stroke="${C_LEDGER}" stroke-width="${T}"/>`;
  svg += callout(rSx + 25, rSy - 5, `BIRDSMOUTH ${birdsmouth}mm`, rSx + 8, rSy);

  // M12 bolt
  svg += `<circle cx="${rSx + perpX * 0.5 + 10 * Math.cos(angle)}" cy="${baseY - ledD / 2}" r="${boltDia * 0.15}" fill="none" stroke="${C_BOLT}" stroke-width="1"/>`;
  svg += callout(rSx + 30, baseY - ledD / 2 - 12, `${p.boltThroughSize || 'M12'} BOLT`, rSx + 22, baseY - ledD / 2);
  ...
  // Pitch angle arc
  const arcR = 30;
  svg += `<path d="M ${cx + arcR} ${baseY - ledD - arcR} A ${arcR} ${arcR} 0 0 1 ${cx + arcR * Math.cos(angle)} ${baseY - ledD - arcR * Math.sin(angle)}" fill="none" stroke="${C_DIM}" stroke-width="${F}"/>`;
  svg += `<text x="${cx + arcR + 5}" y="${baseY - ledD - arcR / 2}" fill="${C_DIM}" font-family="${mono}" font-size="9">${pitch}°</text>`;
